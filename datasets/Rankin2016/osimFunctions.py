...
def addTorqueActuators(osimModel = None,
    ...
    Input:    osimModel - OpenSim model object for use
			  ...
			  controlLimits - dict of coordinates and their associated max/min control limits to add (default = inf)
...
def kinematicsToStates(kinematicsFileName = None, osimModelFileName = None,
    ...
    #Read back in as time series table to more easily delete columns
    statesTable = osim.TimeSeriesTable(outputFileName)
    
    #Look for and remove any columns that haven't been converted to state format
    for ii in range(angleNames.getSize()):
        if not angleNames.get(ii).endswith('/value') and angleNames.get(ii) != 'time':
            statesTable.removeColumn(angleNames.get(ii))
